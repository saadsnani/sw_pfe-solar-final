...
async function readControlSnapshot(controlUrl) {
  ...
  const relaysRaw =
    typeof data.relays === "object" && data.relays !== null
      ? data.relays
      : {};

  const mode = typeof data.mode === "string" ? data.mode : "unknown";
  const inverter = toBool(relaysRaw.inverter ?? data.inverter);
  const block1 = toBool(relaysRaw.block1 ?? data.block1);
  const block2 = toBool(relaysRaw.block2 ?? data.block2);
